...
def isGammaAcyclic (α : Type) [DecidableEq α] (hg : ComputableHypergraph α) : Bool :=
  let rec loop (g : ComputableHypergraph α) (iter : Nat) : ComputableHypergraph α :=
    ...
    | val + 1 =>
      ...
      let newHyperEdges := g.hyperedges.filter (λ e => (g.hyperedges.filter (λ f => (e ⊆ f ∨ e ∩ f = ∅) ∧ e ≠ f)).card >= 1)
...
-- Below are test --
--------------------
#eval findVerticesNoHyperEdge ℕ braultBaronBHyperGraph ∪ findVerticesOneHyperEdge ℕ braultBaronBHyperGraph
